...
Запуск:  1)клонирование репозитория  2)запуск через среду разработки(рекомендую IntellijIdea) или командно:

bash:  ./gradlew run --args="interactionFiles/input.csv interactionFiles/output.csv"

Если не указывать аргументы, программа использует пути по умолчанию:  interactionFiles/input.csv — входной файл  interactionFiles/output.csv — выходной файл
...
Если количество товаров == 0 то можно удалить, нет смысла хранить.
